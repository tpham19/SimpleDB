...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        // Check if it is an equality join
        ...
        // Returns a fixed fraction of the cross-product 
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= joins.size(); i++) {
            Set<Set<LogicalJoinNode>> setOfSubsets = this.enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> subset : setOfSubsets) {
                ...
                for (LogicalJoinNode joinToRemove : subset) {
                    CostCard currentPlan = this.computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, subset, bestPlan.cost, planCache);
                    if (currentPlan != null && currentPlan.cost < bestPlan.cost) {
                        bestPlan = currentPlan;
                    }
                }
                // Add the best plan to the plan cache
                planCache.addPlan(subset, bestPlan.cost, bestPlan.card, bestPlan.plan);
            }
        }
        Set<LogicalJoinNode> logicalJoinNodes = new HashSet<LogicalJoinNode>(joins);
        Vector<LogicalJoinNode> bestLogicalJoinNodesOrdering = planCache.getOrder(logicalJoinNodes);
        // Checks if the explain flag is true 
        if (explain) {
            this.printJoins(bestLogicalJoinNodesOrdering, planCache, stats, filterSelectivities);
        }
        return bestLogicalJoinNodesOrdering; 
    }
