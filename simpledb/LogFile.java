    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                preAppend();
                // some code goes here
                long transactionIdValue = tid.getId();
                this.rollback(transactionIdValue);
            }
        }
    }

    private void rollback(long transactionIdValue)
        throws NoSuchElementException, IOException {
        if (tidToFirstLogRecord.containsKey(transactionIdValue)) {
            long transactionOffset = tidToFirstLogRecord.get(transactionIdValue);
            // Get the current pointer value and then update endingOffset to read the log in reverse
            long endingOffset = raf.getFilePointer();
            while (transactionOffset < endingOffset) {
                // Go back by a long size to read the starting offset of a log record
                raf.seek(endingOffset - LONG_SIZE);
                endingOffset = raf.readLong();
                // Go to the start of the log record
                raf.seek(endingOffset);
                int recordType = raf.readInt();
                long recordTransactionIdValue = raf.readLong();
                if (recordTransactionIdValue == transactionIdValue && recordType == UPDATE_RECORD) {
                    Page beforeImage = this.readPageData(raf);
                    PageId beforeImagePageId = beforeImage.getId();
                    Database.getCatalog().getDatabaseFile(beforeImagePageId.getTableId()).writePage(beforeImage);
                } else if (recordTransactionIdValue == transactionIdValue && recordType == COMMIT_RECORD) {
                    throw new IOException();
                }
            }
        }
        // Move the pointer back to the currentOffset position
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // Clear and then update tidToFirstLogRecord from
                // the last checkpoint or from the start of the file.
                tidToFirstLogRecord.clear();
                if (lastCheckpointOffset != NO_CHECKPOINT_ID) {
                    raf.seek(lastCheckpointOffset);
                    int checkpointType = raf.readInt();
                    if (checkpointType != CHECKPOINT_RECORD) {
                        throw new RuntimeException("The file pointer does not point to a checkpoint record.");
                    }
                    raf.readLong();
                    int numberOfTransactions = raf.readInt();
                    for (int i = 0; i < numberOfTransactions; i++) {
                        long transactionIdValue = raf.readLong();
                        long firstRecordOffset = raf.readLong();
                        tidToFirstLogRecord.put(transactionIdValue, firstRecordOffset);
                    }
                    raf.readLong();
                }
                ...
                while (startingOffset < lengthOfFile) {  
                    int recordType = raf.readInt();
                    long recordTransactionIdValue = raf.readLong();
                    switch (recordType) {
                        case UPDATE_RECORD:
                            ...
                            PageId afterImagePageId = afterImage.getId();
                            Database.getCatalog().getDatabaseFile(afterImagePageId.getTableId()).writePage(afterImage);
                            break;
                        case ABORT_RECORD:
                            tidToFirstLogRecord.remove(recordTransactionIdValue);
                            break;
                        case COMMIT_RECORD:
                            tidToFirstLogRecord.remove(recordTransactionIdValue);
                            break;
                        ...
                        case BEGIN_RECORD:
                            tidToFirstLogRecord.put(recordTransactionIdValue, startingOffset);
                            break;
                        default:
                            break;  
                    }
                    raf.readLong();
                    startingOffset = raf.getFilePointer();       
                }
                currentOffset = raf.getFilePointer();
                for (long transactionIdValue : tidToFirstLogRecord.keySet()) {
                    this.rollback(transactionIdValue);
                }
            }
        }
